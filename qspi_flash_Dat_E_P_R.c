...
static int qspi_flash_wait_ready(struct qspi_flash *qf)
{
	...
	while( timeout ) {
		...
		DPRINTK("%s %d --> status[0] = %x status[1] = %x\n", __func__, __LINE__,status[0], status[1]);
...
static int qspi_flash_do_read(struct qspi_flash *qf, u8 *buf, int len)
{
	...
	DPRINTK("%s %d", __func__, __LINE__);
	 
	print_hex_dump(KERN_ALERT, "data: ", DUMP_PREFIX_NONE, 16, 1, buf, len, false);
#endif /*RZA2MEVB */
...
static int qspi_flash_do_erase(struct qspi_flash *qf)
{
	...
	print_hex_dump(KERN_ALERT, "data: ", DUMP_PREFIX_NONE, 16, 1, debug_buf, length, false);
...
static ssize_t qspi_flash_write(struct file * file, const char __user * buf,  size_t count, loff_t *ppos)
{
	...
	print_hex_dump(KERN_ALERT, "data: ", DUMP_PREFIX_NONE, 16, 1, in_buf, count, false);
#endif /* DEBUG */
...
#if RZA2MEVB
	mmio_id = ioremap(0x20000000, 0x1000000);
	if (mmio_id == NULL) {
		ret = -ENOMEM;
		printk("mmio_id ioremap error.\n");
		goto clean_up;
	}
