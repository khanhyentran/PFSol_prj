...
// 10      4 bytes  Offset bits
    54,0,0,0,
// 14      4 bytes  Header (BITMAPINFOHEADER) size (n)
    40,0,0,0,
// 18      4 bytes  Width
    0,0,0,0,
// 22      4 bytes  Height
...
uint8_t * 
put_bmp(uint8_t * rgb, uint32_t width, uint32_t height, uint16_t bpp)
{
    uint32_t *ptr32;
    uint16_t *ptr16;

    // total fize size
    ptr32 = (uint32_t *) (&bmp_header[2]);
    *ptr32 = width * height * bpp/8 + BMP_HDR_SIZE;

    // width
    ptr32 = (uint32_t *) (&bmp_header[18]);
    *ptr32 = width;

    // height
    ptr32 = (uint32_t *) (&bmp_header[22]);
    *ptr32 = height;

    // bpp
    ptr16 = (uint16_t *) (&bmp_header[28]);
    *ptr16 = bpp;

    memcpy(rgb, bmp_header, BMP_HDR_SIZE);

    return rgb + BMP_HDR_SIZE;
};
...
main( int argc, char ** argv )
{
    ...
    while ((c=getopt_long(argc,argv,"i:o:w:v:b:f:m:tphs",longopt,&optidx)) != (char)-1)
    {
        switch ( c )
        {
            case 'i':
    ...
    out_size = width * height * (bpp / 8) * 3 + tiff + bmp;
    ...
    if(bmp)
    {
        rgb_start = put_bmp(rgb, width, height, 24);
    }
#if 1
    switch(bpp)
    {
...
//            ((int*)rgb)[2] = 0xadadadad;
    ...
    if ( bmp && (width > 1280) ) {
        printf("\nError: Image width must be less thatn 1280\n\n");
        bmp = 0;
    }
    ...
    if (bmp) {
        ...
        /* modify the file to display bottom line first */
        front_index = BMP_HDR_SIZE;
        end_index = width * 3 * (height - 1) + BMP_HDR_SIZE;
        for (row=0 ; row < height/2; row++)
        {
            /* copy front line to tmp */
            memcpy(tmp_line, rgb + front_index, width * 3);

            /* copy end to front */
            memcpy(rgb + front_index, rgb + end_index, width * 3);

            /* copy tmp to end */
            memcpy(rgb + end_index, tmp_line, width * 3);
