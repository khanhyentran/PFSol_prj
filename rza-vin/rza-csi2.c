...
//difference in bit of interrupt
/* Interrupt Enable */
...
static int rcar_csi2_calc_phy_timing(struct rcar_csi2 *priv)
{
	...
	rcar_csi2_write(priv, PHYTIM2_REG, PHYTIM2_TCLK_PRELARE(0xA) | PHYTIM2_TCLK_SETTLE(0xF) | PHYTIM2_TCLK_MISS(0x3));
...
static int rcar_csi2_start(struct rcar_csi2 *priv, struct v4l2_subdev *nextsd)
{
	...
	/* Configure */
	/* check "Input data is interfaced?"*/
	/*tmp = rcar_csi2_read(priv, PHYDIM);
	if(tmp == 0)
	{
