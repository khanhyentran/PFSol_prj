...
static int rvin_digital_notify_complete(struct v4l2_async_notifier *notifier)
{
	...
	int ret;
printk("digital call v4l2_device_register_subdev_nodes \n");
...
static int rvin_group_notify_complete(struct v4l2_async_notifier *notifier)
{
	...
	int ret;
RVC_log();
printk("rvin_group_notify_complete call v4l2_device_register_subdev_nodes %x\n", &vin->v4l2_dev);
	ret = v4l2_device_register_subdev_nodes(&vin->v4l2_dev);
	if (ret) {
		vin_err(vin, "Failed to register subdev nodes\n");
		return ret;
...
RVC_log();
	...
	mutex_unlock(&vin->group->lock);
	vin_err(vin, "Call Notifier registration\n");//RVC
	ret = v4l2_async_notifier_register(&vin->v4l2_dev, &vin->notifier);
	if (ret < 0)
		vin_err(vin, "Notifier registration failed\n");
	...
	if (vin->info->use_mc) {
		while (1) {
			...
			if (enable)
				break;
			if (!timeout) {
				dev_warn(vin->dev, "MSTP status timeout khanh\n");
				break;
			}
			usleep_range(10, 15);
			timeout--;
