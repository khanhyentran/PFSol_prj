...
#define VNUTP_REG	0x100 /*Video n  lookup table pointer*///RVC add
#define VNUTD_REG	0x104 /*Video n  lookup table data register*///RVC add
#define VNYCCR(m)_REG	(0x224 + (m) << 2) /*Video RGB -> Y calculation setting register*///RVC add
...
RVC_log();
	...
	printk("VNMC_REG %x",rvin_read(vin, VNMC_REG));
	rvin_write(vin, regs.ctrl, VNUDS_CTRL_REG);
	printk("VNUDS_CTRL_REG %x",rvin_read(vin, VNUDS_CTRL_REG));
	rvin_write(vin, regs.scale, VNUDS_SCALE_REG);
	printk("VNUDS_SCALE_REG %x",rvin_read(vin, VNUDS_SCALE_REG));
	rvin_write(vin, regs.pass_bwidth, VNUDS_PASS_BWIDTH_REG);
	printk("VNUDS_PASS_BWIDTH_REG %x",rvin_read(vin, VNUDS_PASS_BWIDTH_REG));
	rvin_write(vin, regs.clip_size, VNUDS_CLIP_SIZE_REG);
	printk("VNUDS_CLIP_SIZE_REG %x",rvin_read(vin, VNUDS_CLIP_SIZE_REG));
...
RVC_log();
	...
	case V4L2_FIELD_INTERLACED_BT:
		...
		printk("VNSLPRC_REG %x",rvin_read(vin, VNSLPRC_REG));
		rvin_write(vin, (vin->crop.top + vin->crop.height) / 2 - 1,
			   VNELPRC_REG);
		printk("VNELPRC_REG %x",rvin_read(vin, VNELPRC_REG));
		break;
	default:
	...
	printk("VNIS_REG %x",rvin_read(vin, VNIS_REG));
}
...
RVC_log();
	...
	printk("VNINTS_REG %x",rvin_read(vin, VNINTS_REG));
	/* Enable interrupts */
	rvin_write(vin, interrupts, VNIE_REG);
	printk("VNIE_REG %x",rvin_read(vin, VNIE_REG));
	/* Start capturing */
	rvin_write(vin, dmr, VNDMR_REG);
printk("VNDMR_REG %x",rvin_read(vin, VNDMR_REG));
	rvin_write(vin, dmr2, VNDMR2_REG);
printk("VNDMR2_REG %x",rvin_read(vin, VNDMR2_REG));
...
{
RVC_log();
printk("VNMC_REG %x",rvin_read(vin, VNINTS_REG));
...
{
RVC_log();
printk("VNMC_REG %x",rvin_read(vin, VNMS_REG));	
...
printk("VNFC_REG %x",rvin_read(vin, VNFC_REG));	
RVC_log();
	if (vin->info->chip == RCAR_GEN3) {
		u32 vnmc;
...
printk("rvin_dma_probe call v4l2_device_register %x ", &vin->v4l2_dev);
...
RVC_log();
	...
	printk("VNCSI_IFMD_REG %x",rvin_read(vin, VNCSI_IFMD_REG));	
