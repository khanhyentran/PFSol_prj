...
static int __init qspi_flash_init(void)
{
	...
	mmio = ioremap(0x20000000, 0x1000000);
	if (mmio == NULL) {
		ret = -ENOMEM;
	...
	printk("in XIP mode 1, QSPI_CMNSR = %x\n",qspi_read32(&my_qf, QSPI_CMNSR));

	qspi_write32(&my_qf, 0x00fff300, QSPI_CMNCR);

	qspi_write32(&my_qf, 0x00070707, QSPI_SSLDR);
	
	qspi_write32(&my_qf, DRCR_RCF | DRCR_RBE | DRCR_SSLE | DRCR_RBURST(1), QSPI_DRCR);
	
	qspi_read32(&my_qf, QSPI_DRCR);//KYT dummy read

	qspi_write32(&my_qf, DRCMR_CMD(0x9F), QSPI_DRCMR);
	
	//qspi_write32(&my_qf, 0x00000000, QSPI_DREAR);
	//qspi_write32(&my_qf, 0x00000000, QSPI_DROPR);
	qspi_write32(&my_qf, 0x00004000, QSPI_DRENR);
	//qspi_write32(&my_qf, 0x00000000, QSPI_DRDMCR);
	//qspi_write32(&my_qf, 0x00000111, QSPI_DRDRENR);

	to_show_register_value(&my_qf);
printk("in XIP mode 2, QSPI_CMNSR = %x\n",qspi_read32(&my_qf, QSPI_CMNSR));
	memcpy(id, mmio + 0, sizeof(u8) * 5);
