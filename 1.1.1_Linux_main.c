...
//
    	...
    	ret = rpmsg_send(&cb_rp_ept_test, i_payload, (2 * sizeof(unsigned long)) + size);
    	if (ret < 0) {
    	    LPERROR("rdev = NULL -> rpmsg_send failed %d\n", ret);
    	    if (ret == RPMSG_ERR_PARAM)
    	        printf("TC 1.1.2 is PASS\n");
    	    else 
    	        printf("TC 1.1.2 is FAILED\n");
    	}

    	/* $$$$ Test Point $$$$ */
    	cb_rp_ept_test.dest_addr = RPMSG_ADDR_ANY;
    	ret = rpmsg_send(&cb_rp_ept_test, i_payload, (2 * sizeof(unsigned long)) + size);
    	if (ret < 0) {
    	    LPERROR("dest_addr = RPMSG_ADDR_ANY -> rpmsg_send failed %d\n", ret);
    	...
    	/* $$$$ Test Point $$$$ */
    	ret = rpmsg_send(&rp_ept, NULL, (2 * sizeof(unsigned long)) + size);
    	if (ret < 0) {
	    LPERROR("data = NULL -> rpmsg_send failed %d\n", ret);
            if (ret == RPMSG_ERR_PARAM)
            	printf("TC 1.1.3 is PASS\n");
	    else 
        	printf("TC 1.1.3 is FAILED\n");
    	}
        ...
        LPRINTF("TC 1.1.1 is PASS, echo test: sent : %lu\n", (2 * sizeof(unsigned long)) + size);
