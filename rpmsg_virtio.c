...
{
	void *buffer;
	printf("rpmsg_virtio_shm_pool_get_buffer 1\n");
	if (shpool->avail < size){
		printf("rpmsg_virtio_shm_pool_get_buffer shpool->avail < size\n");
		return NULL;
		}
	
	printf("rpmsg_virtio_shm_pool_get_buffer 2 shpool->avail %d\n", shpool->avail);
	buffer =  (void *)((char *)shpool->base + shpool->size - shpool->avail);
	shpool->avail -= size;
	
	printf("rpmsg_virtio_shm_pool_get_buffer 3 shpool->avail %d\n", shpool->avail);
	return buffer;
}
...
printf("rpmsg_virtio_send_offchannel_raw START\n");
	...
	printf("No_buff\n");
		...
		else printf("break while -> buffer is OK\n");
...
printf("done write header\n");
	...
	metal_mutex_acquire(&rdev->lock);
printf("done write buffer\n");
	/* Enqueue buffer on virtqueue. */
	status = rpmsg_virtio_enqueue_buffer(rvdev, buffer, buff_len, idx);
	printf("done write buffer done enqueue_buffer\n");
	RPMSG_ASSERT(status == VQUEUE_SUCCESS, "failed to enqueue buffer\n");
	/* Let the other side know that there is a job to process. */
	virtqueue_kick(rvdev->svq);
	printf("done write buffer done virtqueue_kick\n");
	metal_mutex_release(&rdev->lock);
printf("rpmsg_virtio_send_offchannel_raw END\n");
	return size;
}
