...
static int rpmsg_endpoint_cb0(struct rpmsg_endpoint *cb_rp_ept, void *data, size_t len, uint32_t src, void *priv)
{
    ...
    ret = rpmsg_send(cb_rp_ept_test, data, (int)len);
    if (ret < 0) {
        LPERROR("rdev = NULL -> rpmsg_send failed %d\n", ret);
        if (ret == RPMSG_ERR_PARAM)
            printf_raw("TC 1.2.2 is PASS\n");
        else 
            printf_raw("TC 1.2.2 is FAILED\n");
    }
    ...
    /* $$$$ Test Point $$$$ */
    ret = rpmsg_send(cb_rp_ept, NULL, (int)len);
    if (ret < 0) {
        LPERROR("data = NULL -> rpmsg_send failed %d\n", ret);
        if (ret == RPMSG_ERR_PARAM)
            printf_raw("TC 1.2.3 is PASS\n");
        else 
            printf_raw("TC 1.2.3 is FAILED\n");
    }

    /* $$$$ Test Point 1.2.1 $$$$ */
    /* Send data back to master */
    *(unsigned int *)data = SHUTDOWN_MSG;
    if (rpmsg_send(cb_rp_ept, data, (int)len) < 0) {
        LPERROR("rpmsg_send failed\n");
        return -1;
    }
    else
        printf_raw("rpmsg_send -> return RPMSG_SUCCESS\n TC 1.2.1 is PASS\n");
