...
/* Refer from CEU for memory address - single mode first, try continuous mode later */
static unsigned int cap_buf_addr = 0x40200000;/* offset in internal RAM or HyperRAM*/
...
/* globals; refer from CEU for memory map */
struct mem_map {
	char name[20];
	void *base;
	uint32_t addr;
...
struct mem_map map_mipi, map_vin, map_ram;
int mem_fd;

static void save_image(void/*char *buf*/)
{
	int fd;
	char *filename;
	int ret = 0;
	...
	ret =  write(fd, map_ram.base, 384000);
	if (ret < 0){
		printf("write error \n");
		ret =  write(fd, "abcd", 4);
		if (ret < 0){
			printf("write abcd error \n");
		}
	}
	else if(ret != (384000))
