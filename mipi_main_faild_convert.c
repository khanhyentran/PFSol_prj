...
static unsigned int cap_buf_addr1 =  0x40200000;//0x4025DC00;//0x40200000;/* offset in internal RAM or HyperRAM*/
...
struct mem_map map_mipi, map_vin, map_ram1, map_ram2;
int mem_fd;
...
static void save_image(void)
{
	...
	ret =  write(fd, map_ram2.base, 384000);
	if (ret < 0){
		printf("write error \n");
		}
	else if(ret != (384000))
...
int close_resources(void)
{
	...
	if( map_ram2.base ) {
		munmap(map_ram2.base, map_ram2.size);
		map_ram2.base = 0;
	}
}
...
int open_resources(void)
{
	...
	{
		map_ram1.base = NULL;
		perror("Could not map memory1 for captured first frame");
		close(mem_fd);
		return -1;
	}
#if 0
	map_ram2.addr = cap_buf_addr2;
	map_ram2.size = cap_buf_size;

	map_ram2.base = mmap(NULL, map_ram2.size,
			  PROT_READ | PROT_WRITE,
			  MAP_SHARED, mem_fd, 
			  map_ram2.addr );
	if (map_ram2.base == MAP_FAILED)
	{
		map_ram2.base = NULL;
		perror("Could not map memory2 for captured the second frame");
		close(mem_fd);
		return -1;
	}
#endif
	...
	//printf("map_ram2.size=0x%lx\n", map_ram2.size);
	//printf("map_ram2.addr=0x%lx\n", map_ram2.addr);
	//printf("map_ram2.base=%p\n", map_ram2.base);
#endif
...
void main(void)
{
	...
	//retval = R_MIPI_SetBufferAdr(1, map_ram2.addr);
