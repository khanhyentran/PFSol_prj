...
int app (struct rpmsg_device *rdev, void *priv, unsigned long svcno)
{
    ...
    for (i = 0, size = pi.min; i < 1; i++, size++) {
        i_payload->num = i;
        i_payload->size = size;
...
//
    	...
    	ret = rpmsg_trysend_offchannel(&cb_rp_ept_test, src, dst, i_payload, (2 * sizeof(unsigned long)) + size);
    	if (ret < 0) {
    	    LPERROR("rdev = NULL -> rpmsg_trysend_offchannel failed %d\n", ret);
    	    if (ret == RPMSG_ERR_PARAM)
    	        printf("TC 7.1.2 is PASS\n");
    	    else 
    	        printf("TC 7.1.2 is FAILED\n");
    	}

    	/* $$$$ Test Point $$$$ */
    	
    	ret = rpmsg_trysend_offchannel(&cb_rp_ept_test, src, RPMSG_ADDR_ANY, i_payload, (2 * sizeof(unsigned long)) + size);
    	if (ret < 0) {
    	...
    	/* $$$$ Test Point $$$$ */
    	ret = rpmsg_trysend_offchannel(&rp_ept, src, dst, NULL, (2 * sizeof(unsigned long)) + size);
    	if (ret < 0) {
	    LPERROR("data = NULL -> rpmsg_trysend_offchannel failed %d\n", ret);
            if (ret == RPMSG_ERR_PARAM)
            	printf("TC 7.1.3 is PASS\n");
	    else 
        	printf("TC 7.1.3 is FAILED\n");
    	}

   	 /* $$$$ Test Point 7.1.1 $$$$ */
   	 /* Send data back to master */

        ret = rpmsg_trysend_offchannel(&rp_ept, src, dst, i_payload,
             (2 * sizeof(unsigned long)) + size);
     
        if (ret < 0) {
            LPRINTF("Error sending data...%d, TC 1.1.1 is FAILED\n", ret);
        break;
        }
        LPRINTF("TC 7.1.1 is PASS, echo test: sent : %lu\n", (2 * sizeof(unsigned long)) + size);
