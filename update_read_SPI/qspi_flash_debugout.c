...
#if RZA2MEVB
		scif_base = ioremap_nocache(0xe8009000, 0x30); /* SCIF4 */
		#define SCFTDR *(volatile uint8_t *)(scif_base + 0x06)
		#define SCSCR *(volatile uint16_t *)(scif_base + 0x04)
		#define SCBRR *(volatile uint8_t *)(scif_base + 0x02)
		#define SCFSR *(volatile uint16_t *)(scif_base + 0x08)
		#define SCFCR *(volatile uint16_t *)(scif_base + 0x0C)
		#define SCFDR *(volatile uint16_t *)(scif_base + 0x0E)
#else
...
static int __init qspi_flash_init(void)
{
	...
	/* Macronix MX25L51245G */
	DPRINTK("Flash ID = %02X %02X %02X %02X %02X\n",id[0],id[1],id[2],id[3],id[4]);
	/* RZ/A2MEVB : Flash ID = FE 20 1A C2 00 *
	 * 		  FE : Wrong data				 *
	 * 		  20 : Memory type				 *
	 * 		  1A : Memory density 			 *
	 * 		  C2 : CManufacturer ID			 *
