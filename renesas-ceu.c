...
static int ceu_parse_dt(struct ceu_device *ceudev)
{
	...
	int num_ep;
	int ret;
printk("ceu_parse_dt  of_graph_get_endpoint_count\n");
	num_ep = of_graph_get_endpoint_count(of);
	if (!num_ep)
		return -ENODEV;
printk("ceu_parse_dt ceu_init_async_subdevs\n");
	...
	for (i = 0; i < num_ep; i++) {
		printk("ceu_parse_dt of_graph_get_endpoint_by_regs\n");
	...
	printk("ceu_parse_dt v4l2_fwnode_endpoint_parse\n");
...
static const struct of_device_id ceu_of_match[] = {
	...
	{ .compatible = "renesas,r7s9210-ceu", .data = &ceu_data_rz },
	{ }
};
...
static int ceu_probe(struct platform_device *pdev)
{
	...
	ceudev = devm_kzalloc(dev,sizeof(*ceudev), GFP_KERNEL);
	if (!ceudev)
		return -ENOMEM;
	...
	ceudev->notifier.complete = ceu_notify_ops.complete;
	ceudev->notifier.bound = ceu_notify_ops.bound;
