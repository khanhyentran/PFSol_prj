...
RVC_log();
	...
	printk("SRST_REG %x",rcar_csi2_read(priv, SRST_REG));
}
...
printk("PHYTIM3_REG %x",rcar_csi2_read(priv, PHYTIM3_REG));
printk("PHYTIM2_REG %x",rcar_csi2_read(priv, PHYTIM2_REG));
printk("PHYTIM1_REG %x",rcar_csi2_read(priv, PHYTIM1_REG));

/* Non OS source for CAM_IU233
	...
	rcar_csi2_write(priv, PHYTIM1_REG, PHYTIM1_T_INIT_SLAVE(0x338F));
*/
/* Non OS source for CAM_IMX219_B
	rcar_csi2_write(priv, PHYTIM3_REG, PHYTIM3_THS_PREPARE(18) | PHYTIM3_THS_SETTLE(25));
	rcar_csi2_write(priv, PHYTIM2_REG, PHYTIM2_TCLK_PREPARE(15) | PHYTIM2_TCLK_SETTLE(30) | PHYTIM2_TCLK_MISS(8));
	rcar_csi2_write(priv, PHYTIM1_REG, PHYTIM1_T_INIT_SLAVE(15199));
*/
/* Non OS source for CAM_IMX219_Q
	rcar_csi2_write(priv, PHYTIM3_REG, PHYTIM3_THS_PREPARE(3) | PHYTIM3_THS_SETTLE(12));
	rcar_csi2_write(priv, PHYTIM2_REG, PHYTIM2_TCLK_PREPARE(3) | PHYTIM2_TCLK_SETTLE(15) | PHYTIM2_TCLK_MISS(3));
	rcar_csi2_write(priv, PHYTIM1_REG, PHYTIM1_T_INIT_SLAVE(15199));
*/
	return 0;
}
...
printk("Check virtual channel setting %x", tmp);
		/* Store in correct reg and offset */
		if (i < 2){
			vcdt |= tmp << ((i % 2) * 16);
			printk("<2 Check virtual channel setting %x", vcdt);
		}
		else{
			vcdt2 |= tmp << ((i % 2) * 16);
			printk(">2 Check virtual channel setting %x", vcdt2);
		}
	}
	...
	if (priv->info->clear_ulps)
		rcar_csi2_write(priv, INTSTATE_REG,
				INTSTATE_INT_ULPS_START |
				INTSTATE_INT_ULPS_END);
printk("INTSTATE_REG %x",rcar_csi2_read(priv, INTSTATE_REG));
	/* Init */
	rcar_csi2_write(priv, TREF_REG, TREF_TREF);
printk("TREF_REG %x",rcar_csi2_read(priv, TREF_REG));
	rcar_csi2_reset(priv);

	/* do not check "Input data is interlaced?"*/
	//rcar_csi2_write(priv, FLD_REG, fld | FLD_FLD_EN);//RVC follow NON-OS
printk("FLD_REG %x",rcar_csi2_read(priv, FLD_REG)); //=0
	vcdt = 0x11e802a;
	rcar_csi2_write(priv, VCDT_REG, vcdt);
printk("VCDT_REG %x",rcar_csi2_read(priv, VCDT_REG));
	/* Lanes are zero indexed */
	rcar_csi2_write(priv, LSWAP_REG,
			LSWAP_L0SEL(priv->lane_swap[0] - 1) |
			LSWAP_L1SEL(priv->lane_swap[1] - 1));
printk("LSWAP_REG %x",rcar_csi2_read(priv, LSWAP_REG));
	rcar_csi2_write(priv, PHYCNT_REG, phycnt);
printk("PHYCNT_REG %x",rcar_csi2_read(priv, PHYCNT_REG));
	rcar_csi2_write(priv, PHYCNT_REG, LINKCNT_MONITOR_EN |
