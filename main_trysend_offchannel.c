...
int app (struct rpmsg_device *rdev, void *priv, unsigned long svcno)
{
    ...
    uint32_t dest = rp_ept.dest_addr;
    ...
    for (i = 0, size = 480/*pi.min*/; i < (int)pi.num; i++/*, size++*/) {
        i_payload->num = i;
        i_payload->size = size;
        ...
        LPRINTF("K sending payload number %lu of size %lu\n",
             i_payload->num, (2 * sizeof(unsigned long)) + size);
     
        //ret = rpmsg_trysend_offchannel(&rp_ept, rp_ept.addr, rp_ept.dest_addr, i_payload,
        //     (2 * sizeof(unsigned long)) + size); /*OK*/
	//ret = rpmsg_trysend_offchannel(&rp_ept, 0x501, 0x502, i_payload,
        //     (2 * sizeof(unsigned long)) + size); /*OK*/

	ret = rpmsg_trysend_offchannel(&rp_ept, 0x501, 0x502, NULL,
             (2 * sizeof(unsigned long)) + size); /*error PARAM*/
     
        if (ret < 0) {
            LPRINTF("Error sending data PARAM...%d\n", ret);
#if 1
	    ret = rpmsg_trysend_offchannel(&rp_ept, 0x501, RPMSG_ADDR_ANY, i_payload,
             (2 * sizeof(unsigned long)) + size); /*error ADDR*/

		if (ret < 0) {
            		LPRINTF("Error sending data ADDR...%d\n", ret);
			ret = rpmsg_trysend_offchannel(&rp_ept, RPMSG_ADDR_ANY, 0x502, i_payload,
            			 (2 * sizeof(unsigned long)) + size); /*error ADDR*/
